...
class UserController extends Controller
{
    ...
    public function register(Request $request): JsonResponse
    {
        ...
        try {
            ...
            // allow only one doctor
            if ($request->level_of_authorization == User::USER_ROLE_DOCTOR) {
                $existing_doctor = User::where('level_of_authorization', User::USER_ROLE_DOCTOR)->count();
                if ($existing_doctor >= 2) {
                    return $this->sendError('A doctor account already exists. Only two doctor accounts are allowed.', [], 400);
                }
            }
    ...
    public function list(Request $request): JsonResponse
    {
        ...
        try {
            ...
            $users = User::when($keyword, function ($query, $keyword) {
                ...
                ->where('level_of_authorization', $request->get('user_type', 1))
                ->paginate($per_page);

            // now we need to append the secretary information for each doctor
            $users->getCollection()->transform(function ($user) {
                if ($user->level_of_authorization == 1) { // if the user is a doctor
    ...
    public function addSecretary(Request $request): JsonResponse
    {
        try {
            $user = Auth::user();
            ...
            if (!$user->secretaries()->where('assistant_id', $secretary->id)->exists()) {
                $user->secretaries()->attach($secretary->id);
            } else {
                return $this->sendError('This user is already your secretary', [], 400);
            }
