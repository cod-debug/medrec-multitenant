<template>
    <q-page class="q-pa-md">
        <q-card>
            <q-card-section>
                <div class="text-h6"> <q-icon name="medication" /> Generic Medicines</div>
            </q-card-section>
            <q-separator />
            <q-card-section>
                <div>
                    <q-btn label="Add Generic Medicine" color="primary" icon="add" size="sm" class="q-mb-md" @click="showAddDialog = true" />
                    
                    <div class="row">
                        <div class="col-12 col-md-6">
                            <q-form @submit="getList(true)" class="q-pb-md">
                                <q-input label="Search Generic Medicine" outlined dense v-model="list_params.search">
                    ...
                    <q-dialog v-model="showAddDialog">
                        <q-card style="min-width: 400px">
                            <q-card-section class="bg-primary text-white">
                                <div class="text-h6">{{ editMode ? 'Edit' : 'Add' }} Generic Medicine</div>
                            </q-card-section>
...
const { listGenericMedicineNames, saveMedicineGenericName, updateGenericMedicineName, deleteGenericMedicineName } = medicine_store;
const { generic_names_list_request } = storeToRefs(medicine_store);

const { proxy } = getCurrentInstance();
const helpers = proxy.$helpers;

const columns = [
    { name: 'name', label: 'Generic Medicine Name', field: 'name', align: 'left' },
...
async function getList(is_search = false) {
    try {
        ...
        fetching.value = true;
        await listGenericMedicineNames(list_params.value);

        const response = generic_names_list_request.value;
...
async function deleteItem(id) {
    try {
        await deleteGenericMedicineName(id);
        const response = medicine_store.generic_name_delete_request;
...
async function saveItem() {
    try {
        ...
        if (editMode.value) {
            await updateGenericMedicineName(formData.value.id, { name: formData.value.name });
            const response = medicine_store.generic_name_update_request;
            ...
            await saveMedicineGenericName({ name: formData.value.name });
            const response = medicine_store.save_generic_name_request;
            
            if (response.success) {
                Notify.create({
                    message: 'Generic medicine saved successfully',
                    position: 'top-right',
                    closeBtn: "X",
                    timeout: 3000,
                    color: 'green',
