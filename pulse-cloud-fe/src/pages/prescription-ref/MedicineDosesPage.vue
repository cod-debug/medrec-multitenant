<template>
    <q-page class="q-pa-md">
        <q-card>
            <q-card-section>
                <div class="text-h6"> <q-icon name="medical_services" /> Medicine Dosage Values</div>
            </q-card-section>
            <q-separator />
            <q-card-section>
                <div>
                    <q-btn label="Add Dosage Value" color="primary" icon="add" size="sm" class="q-mb-md" @click="showAddDialog = true" />
                    
                    <div class="row">
                        <div class="col-12 col-md-6">
                            <q-form @submit="getList(true)" class="q-pb-md">
                                <q-input label="Search Dosage Value" outlined dense v-model="list_params.search">
                    ...
                    <q-dialog v-model="showAddDialog">
                        <q-card style="min-width: 400px">
                            <q-card-section class="bg-primary text-white">
                                <div class="text-h6">{{ editMode ? 'Edit' : 'Add' }} Medicine Dose</div>
                            </q-card-section>
...
const { listMedicineDoses, createMedicineDose, updateMedicineDose, deleteMedicineDose } = medicine_store;
const { medicine_dose_list_request } = storeToRefs(medicine_store);

const { proxy } = getCurrentInstance();
const helpers = proxy.$helpers;

const columns = [
    { name: 'name', label: 'Dosage Value', field: 'name', align: 'left' },
...
async function getList(is_search = false) {
    try {
        ...
        fetching.value = true;
        await listMedicineDoses(list_params.value);

        const response = medicine_dose_list_request.value;
...
async function deleteItem(id) {
    try {
        await deleteMedicineDose(id);
        const response = medicine_store.medicine_dose_delete_request;
...
async function saveItem() {
    try {
        ...
        if (editMode.value) {
            await updateMedicineDose(formData.value.id, { name: formData.value.name });
            const response = medicine_store.medicine_dose_update_request;
            ...
            await createMedicineDose({ name: formData.value.name });
            const response = medicine_store.medicine_dose_create_request;
            
            if (response.success) {
                Notify.create({
                    message: 'Medicine dose saved successfully',
                    position: 'top-right',
                    closeBtn: "X",
                    timeout: 3000,
                    color: 'green',
