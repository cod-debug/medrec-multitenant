<template>
    <q-page class="q-pa-md">
        <q-card>
            <q-card-section>
                <div class="text-h6"> <q-icon name="science" /> Medicine Quantity Preparations</div>
            </q-card-section>
            <q-separator />
            <q-card-section>
                <div>
                    <q-btn label="Add Preparation" color="primary" icon="add" size="sm" class="q-mb-md" @click="showAddDialog = true" />
                    
                    <div class="row">
                        <div class="col-12 col-md-6">
                            <q-form @submit="getList(true)" class="q-pb-md">
                                <q-input label="Search Preparation" outlined dense v-model="list_params.search">
                    ...
                    <q-dialog v-model="showAddDialog">
                        <q-card style="min-width: 400px">
                            <q-card-section class="bg-primary text-white">
                                <div class="text-h6">{{ editMode ? 'Edit' : 'Add' }} Medicine Preparation</div>
                            </q-card-section>
...
const { listMedicinePreparations, createMedicinePreparation, updateMedicinePreparation, deleteMedicinePreparation } = medicine_store;
const { medicine_preparation_list_request } = storeToRefs(medicine_store);

const { proxy } = getCurrentInstance();
const helpers = proxy.$helpers;

const columns = [
    { name: 'name', label: 'Preparation Name', field: 'name', align: 'left' },
...
async function getList(is_search = false) {
    try {
        ...
        fetching.value = true;
        await listMedicinePreparations(list_params.value);

        const response = medicine_preparation_list_request.value;
...
async function deleteItem(id) {
    try {
        await deleteMedicinePreparation(id);
        const response = medicine_store.medicine_preparation_delete_request;

        if (response.success) {
            Notify.create({
                message: 'Medicine preparation deleted successfully',
                position: 'top-right',
                closeBtn: "X",
                timeout: 3000,
                color: 'green',
...
async function saveItem() {
    try {
        ...
        if (editMode.value) {
            await updateMedicinePreparation(formData.value.id, { name: formData.value.name });
            const response = medicine_store.medicine_preparation_update_request;
            
            if (response.success) {
                Notify.create({
                    message: 'Medicine preparation updated successfully',
                    position: 'top-right',
                    closeBtn: "X",
                    timeout: 3000,
                    color: 'green',
            ...
            await createMedicinePreparation({ name: formData.value.name });
            const response = medicine_store.medicine_preparation_create_request;
            
            if (response.success) {
                Notify.create({
                    message: 'Medicine preparation saved successfully',
                    position: 'top-right',
                    closeBtn: "X",
                    timeout: 3000,
                    color: 'green',
